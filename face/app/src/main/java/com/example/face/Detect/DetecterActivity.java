...
public class DetecterActivity extends Activity implements CameraSurfaceView.OnCameraListener, View.OnTouchListener, Camera.AutoFocusCallback, View.OnClickListener {
    ...
    class FRAbsLoop extends AbsLoop {
        ...
        public void loop() {
            if (mImageNV21 != null) {
                final int rotate = mCameraRotate + 180;
                ...
                //匹配度大于0.85（自定义数值）则认为识别通过
                if (max > 0.85f) {
                    Intent paramData = new Intent();
                    paramData.putExtra("name",name);
                    setResult(3,paramData);
                    finish();
                }
            }
